function [r, count, xVector, errorVector, errorRatioVector] = JoshHalleys(f, fp,fpp, x0, TOL, maxI)
% this function will check if Halleys method will converge
% this function takes a twice diff'able function and its fist and second derivative as well as a
...
x1 = x0 - (2*f(x0)*fp(x0))/(2*(fp(x0))^2-f(x0)*fpp(x0)); % create x1 for use in algorithim
count = 1;
xVector = [x0; x1];
error = abs(x1 - x0);
errorVector = error;
...
while (error > TOL) && (count <= maxI)
    ...
    x0 = x1;
    x1 = x1 - (2*f(x0)*fp(x0))/(2*(fp(x0))^2-f(x0)*fpp(x0)); % haylleys algorithim
